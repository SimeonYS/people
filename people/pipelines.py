...
class PeoplePipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `people`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM people WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO people (date, title, link, content)"
